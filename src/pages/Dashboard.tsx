...
<DataBridge onDataReceived={(data) => {
  ...
  const isAlertActive = (sensorName: string) =>
    activeAlertsList.some(alert => alert.sensor === sensorName && !alert.isFixed);

  // Generate alerts with detailed suggestions (only if not already active)
  if (data.temperature > 60 && !isAlertActive("Temperature Sensor")) {
    addAlert({
      id: `temp-${Date.now()}`,
      type: "danger",
  ...
  if (data.current > 2.5 && !isAlertActive("Current Sensor")) {
    addAlert({
      id: `current-${Date.now()}`,
      type: "danger", 
  ...
  if (data.vibration > 20000 && !isAlertActive("Vibration Sensor")) {
    addAlert({
  ...
  if (data.gasEmission > 400 && !isAlertActive("Gas Sensor")) {
    addAlert({
      id: `gas-${Date.now()}`,
      type: "danger",
  ...
  if (data.humidity > 80 && !isAlertActive("Humidity Sensor")) {
    addAlert({
